		...
		<div class="flex flex-wrap items-end gap-2">
			<label class="flex flex-col gap-0.5 text-xs font-medium text-ink-muted">
				...
				<Input
					...
					min={dayMin}
					max={dayMax}
					value={toDateInputValue(shift.startsAt)}
					class="text-sm"
				/>
			</label>
			<label class="flex flex-col gap-0.5 text-xs font-medium text-ink-muted">
				...
				<Input
					name="startTime"
					type="time"
					value={toTimeInputValue(shift.startsAt)}
					class="text-sm"
				/>
			</label>
			<label class="flex flex-col gap-0.5 text-xs font-medium text-ink-muted">
				Fin
				<Input name="endTime" type="time" value={toTimeInputValue(shift.endsAt)} class="text-sm" />
			</label>
			...
			<div class="flex gap-1">
				<button
					type="submit"
					title="Enregistrer"
					class="inline-flex min-h-9 items-center rounded bg-brand-primary px-2 text-white hover:bg-brand-primary-700"
				>
					<Check size={16} />
				</button>
				<button
